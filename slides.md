...
<!--
私はノヴィコフ・アンドレイと申します。10年あまりにルビーもポストグレスキューエルも使って様々なアプリを開発しています。今年、日本へ引越しして、大阪の近くに住んでいます。どうぞよろしくお願いします。
-->
...
<!--
例えば、ふたつの道路標識を比べましょう。左のは、ヨーロッパの交差点でよく見られる標識です。右のは、日本のものです。両方とも「交差点で道を譲るべき」という意味があります。同じようにみえますが、実際にイコールかどうか？
-->
...
<!--
ですが、一つの重要な違いがあるんです。ヨーロッパの場合は、車は止まる必要がありません。他の車や自転車や歩行者がなければ、速度を落とさなくても通行できます。日本の標識は「止まれ」といい、必ず一時停止してから通行するべきです。
-->
...
<!--
本当に同一の標識はこれです。「Stop」の赤い八角形のやつです。そして、2017年から、日本の標識にも英語の「Stop」が書いてありますが、この様な新しい標識はまだけっこう珍しいですね。
...
<!--
でも、データベースの主キーも普段整数です。これは別の話です。大人気のアプリの場合、たくさんの行を挿入したり削除したりすると、二十億のキーは早くなくなるおそれがあります。
...
<!--
次のデータ型は可変精度のあり、浮動小数点数です。普段floatとdoubleといいます。RubyとPostgreSQLは両方ともIEEE754という標準を使っていても、いくつかの微妙な違いがあります。
...
もう一つの例として、二つの「数ではない」値を比較すると、Rubyでは「NaNはNaNにイコールではない」という結果が出ます。これはIEEE754標準に合っています。けれども、PostgreSQLではNaNはNaNにイコールになります。これはバグではなくて、意識的に決めたことです。ご注意ください。
-->
...
Rubyの文字列は実に多目的なものです。サイズも制限されないし、中身もどんなデータでもいいです。文字列ごとは異なるエンコーディングを持てるし、エンコーディングの変更もできます。さらに文字コード的に正しくないデータが入っても問題はありません。

逆に、PostgreSQLには、全ての文字列は同じ文字コードを持つべきです。この文字コードは、データベースの作成時に設定されて、後で変更できません。文字列は、不正なデータやゼロバイトを入れるとエラーが発生します。そして、サイズの制限は１ギガバイトです。
-->
...
<!--
普通のテキストを格納するためにはPostgreSQLのvarcharとtextがいいですけど、ゼロバイトや過ちのある文字列、もしくはバイナリデータを保存するためには、PostgreSQLに特別なbyteaというデータ型があります。

Rubyの対応のデータ型はASCII-8BITというエンコーディングのある文字列です。このデータ型の間に違いは全然ありません、何でも格納できますから。

ですが、ネットワーク転送量とメモリーのオーバーヘッドに気をつけてください。PostgreSQLからバイナリデータを読み込むと書き込むとき、16進書式が使っていますので、データ量が２倍多くなります。

それに、ActiveRecordは各列に型キャスト前の値をメモリに置いていますので、Rubyプロセスのメモリ消費量は３倍大きくなります。１ギガバイトのバイナリ値を読み込むと、ネット上で２ギガバイトが転送されて、Rubyのプロセスメモリで３ギガバイトが使用されますので、ご注意ください。
-->
...
テーブルの列には１ギガバイト以上保存することは絶対できません。PostgreSQLのハードリミットです。

特別なラージオブジェクトという機能を使って、ActiveStorageの添付として、4テラバイトまでのファイルを格納できます。

ですが、ラージオブジェクトも文字列もバイナリ列もJSONなどの大きな値は全部TOASTという技法を使って保存されていますので、パーフォーマンすにすごい影響がありますので、できるだけデータベースに大量のデータを格納しないでください。
-->
...
一番大切のは三番目です。日付のデータ型をSQLクエリの条件では絶対使わないでください。データベースのタイムゾーンはUTCなので、日付の真夜中もUTCですから、結果はUTCオフセットによってずれてしまいます。

注意点がたくさんあるので、詳しくはこのリンクで見てください。
-->
...
離散的な範囲型だったら、例えばinteger又は日付の場合、ActiveRecordは賢いギミックをします。Rubyにはありえない開いた下限値に１を足して、閉じた下限値に変更します。でも、連続的な範囲型だったら、エラーが発生するしか仕方がないんですね。
-->
...
<!--
IPアドレス。ほとんど各アプリがこのデータ型を使います。アクセスログ、監査ログ、ログイン履歴などは普段クライアントのIPアドレスが含められています。IPバージョン４の文字列形式は16バイト以上ですが、PostgreSQLのデータ型は７バイトです。それに専用の演算子と関数がありますので、解析に役に立てるかもしれません。無視しないでください。
-->
