...
Rubyの整数は動的サイズなので、制限がありません。GMPというライブラリを使って、そしてカラツバとトゥームクック乗算アルゴリズムとを使用して、Rubyはほんとうにでかい数を扱えます。プログラマーはオーバーフローについて心配しなくてもいいです。

ただし、逆に、データベースには整数は固定な格納サイズがあります。２バイトのsmallint、４バイトのinteger、８バイトのbigintの間で選べます。

PostgreSQLではintegerの列を作ると、デフォルトでは4バイトの整数データ型の列が作成されます。可能な値はマイナス二十億からプラス二十億ぐらいまで保存できますが、これは普段十分すぎますが…
-->
...
<!--
でも、データベースの主キーも整数です。ここで話は違います。大人気のアプリなら、たくさんの行を挿入したり削除したりすると、二十億の識別子は早くなくなるおそれがあります。
ですので、2017年のリリースされたRuby on Railsバージョン5.1でマイグレーションにテーブルを作成するときのデフォルトの主キーデータ型は4バイトのintegerから８バイトのbigintへかわりました。でも、長い間生きているアプリでは、古いRailsのバージョンで作ったテーブルでは４バイトのintegerの主キーがあるので、気をつけてください！
-->
...
<!--
じゃぁ、大人気、長い生きているアプリを維持する方はいるでしょう。パニックするな！

まず、pgheroのようなツールを使って、integerのオーバーフローに近づいてくるシーケンスがあるかどうか調べてください。

それで、こんなテーブルがあれば、トリガを使って、主キーをbigintやuuidへマイグレーションしてください。

万が一、主キーが既に無くなってしまった場合では、整数は符号付だと覚えてください。この様なギミックを使うとシーケンスをマイナス二十億にリセットできます。これでアプリを直せます、bigintやuuidへのマイグレーションの時間ができます。
-->
