...
<!--
皆さん、こんにちは！今日、Ruby on Railsを使っている開発者はの目でPostgreSQLはどのように見えるかと話したいと思います。
-->
...
<!--
私はノヴィコフ・アンドレイと申します。10年あまりにルビーもポストグレスキューエルも使って様々なアプリを開発しています。今年、日本へ引越しして、今大阪の近くに住んでいます。どうぞよろしくお願いします。
-->
...
<!--
それに、我々はオープンソースの大ファンなので、できる限りオープンソースソフトウェアを使ったり、貢献したり、そしてよく自分のライブラリを作って維持しています。どうぞ自由に使ってください。今は数十の火星で作ったオープンソースプロジェクトが存在しています、このスライドでは一番有名なものです。どうぞ自由に使ってください。
-->
...
<!--
次のデータ型は可変精度のあり、浮動小数点数です。普段floatとdoubleといいます。RubyとPostgreSQLは両方ともIEEE754という標準を使っていますが、いくつかの微妙な違いがあります。
...
<!--
文字列！RubyとPostgreSQLの間の一番大事な違いはエンコーディングのことです。

Rubyの文字列は本当に多目的なものです。サイズも無制限し、中身もどんなでもいいです。文字列ごとは異なるエンコーディングをもってありえますし、文字列を別のエンコーディングに変換できます。それに文字列はエンコーディングに正しくないデータも含まれられます。

PostgreSQLは逆に、全文字列は同じエンコーディング持つはずです。このエンコーディングはデータベースが作成されるときに設定されて、後で変更できません。文字列の中はこのエンコーディングには無効なデータやゼロバイトを入れるとエラーが発生します。そして、列のサイズ制限は１ギガバイトです。

PostgreSQLでは文字列を保存するために三つのデータ型があります。char, varchar, textです。charを使わないほうがいいと思います、格納は遅くて、冗長のスペースも保存されています。詳しくはドキュメントをご覧ください。varcharとtextは本当に同じデータ型です、どちらでも使ってもいいですが。たとえば、SimpleFormというジェムはvarcharの列の場合、HTMLのinputタグを生成して、textの列の場合は複数の行が入力できるtextareaタグを生成します。
-->
...
<!--
現代、Rubyにも、PostgreSQLにも、データ通信にも、どこでも一番多目的と相互運用性のあるユニコードのUTF-8というエンコーディングを使ってください。
-->
...
<!--
普通のテキストを保存するためにはPostgreSQLのvarcharとtextがいいですけど、ゼロバイトや過ちのある文字列、もしくはバイナリデータを保存するためには、PostgreSQLに特別なbyteaというデータ型があります。

Rubyの対応のデータ型はASCII-8BITというエンコーディングのある文字列です。このデータ型の間に違いは全然ありません、何でも保存できますから。

ですが、Rubyのコードからファイルを読み込むとき、特別なbinreadという関数やopenの関数のバイナリフラグを使うのを忘れないでください。バイナリファイルをテキストとして読み込むと内容が破損する可能性がありますので。

もう一つの注意ポイントは、ネットワーク転送とメモリーのオーバーヘッドです。PostgreSQLからバイナリデータを読み込むと書き込むとき、16進書式が使っていますので、データ量が２倍大きくなります。

それに、ActiveRecordは各列に型キャスト前の値をメモリに置いていますので、Rubyプロセスのメモリ消費量は３倍大きくなります。１ギガバイトのバイナリ値を読み込むと、ネット上で２ギガバイトが転送されて、Rubyのプロセスメモリで３ギガバイトを使用しますので、ご注意ください。
-->
