...
PostgreSQLでは、文字列の専用データ型は三つがあります。char, varcharとtextです。charを使わないほうがいいと思います。冗長な空白も保存されているため、入力スピードは低いです。詳しくはマニュアルをご覧ください。varcharとtextは本当に同じデータ型で、どちらも良いです。たとえば、SimpleFormというジェムでは、HTMLのinputタグの内容はvarcharの列に保存されても、textareaタグの方はtextの列に保存する。
-->
